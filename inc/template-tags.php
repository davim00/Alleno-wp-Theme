...
if ( ! function_exists( 'alleno_cv_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function alleno_cv_posted_on() {
	...
	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'alleno-cv' ),
...
if ( ! function_exists( 'alleno_cv_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function alleno_cv_entry_footer() {
 ...
 * @return bool
 */
function alleno_cv_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'alleno_cv_categories' ) ) ) {
		...
		set_transient( 'alleno_cv_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so alleno_cv_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so alleno_cv_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in alleno_cv_categorized_blog.
 */
function alleno_cv_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'alleno_cv_categories' );
}
add_action( 'edit_category', 'alleno_cv_category_transient_flusher' );
add_action( 'save_post',     'alleno_cv_category_transient_flusher' );

/**
 * Dynamic copyright date
 */
 function alleno_cv_copyright() {
	 global $wpdb;
